...
const UserSearch = () => {
    ...
    if (searchBox.length > 0)   {
        ...
        for (let i = 0; i < searchBox.length; i++)  {
            setSearchedUsers((prevState) => prevState.filter((user) =>  {
    ...
    return (
        <>
            ...
            <input type="text" className="validate" onChange={(event) => handleSearch(event)}/>
            </form>
            {searchedUsers.map((user) => (
            <SearchObj key={user._id} username={user.username.toLowerCase()} id={user._id}/>
            ))
            }
